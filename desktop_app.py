...
def init_console():
    """If running as a windowed executable with command-line arguments,
    attach to the parent console to output text.
    """
    if sys.platform == 'win32' and len(sys.argv) > 1:
        if sys.stdout is None:
            import ctypes
            # ATTACH_PARENT_PROCESS is -1
...
def cmd_export(args):
    ...
    abs_scan_path = os.path.abspath(filepath)
    abs_exe_path = os.path.abspath(exe_path)
    abs_guide_path = os.path.abspath(guide_path)

    md = _build_agent_prompt(info, top_files, extensions, cache_suggestions, top_folders, abs_exe_path, abs_guide_path, scan_file, data, abs_scan_path)
    print(md)


def _build_agent_prompt(info, top_files, extensions, cache_suggestions, top_folders, exe_path, guide_path, scan_file, full_data, abs_scan_path):
    """Build the comprehensive agent prompt markdown string."""
    abs_exe_dir = os.path.dirname(exe_path)
    md = ""
    ...
    md += f"- **Diretório do Executável (Workspace):** `{abs_exe_dir}`\n\n"
    ...
    md += f"\"{exe_path}\" export --scan \"{abs_scan_path}\"\n\n"
    md += f"# Comparar scan atual com backup anterior\n"
    md += f"\"{exe_path}\" compare --scan \"{abs_scan_path}\"\n\n"
    ...
    md += f"- **Scan Utilizado (Arquivo):** `{scan_file}`\n"
    md += f"- **Caminho Absoluto do Scan:** `{abs_scan_path}`\n"
    ...
    md += "7. **Compare resultados:** Rode `\"" + str(exe_path) + "\" compare --scan \"" + abs_scan_path + "\"` para mostrar exatamente quanto espaço foi liberado.\n"
